...
def import_fake(x_start, x_split): # x_split datasets, starting at x_start
    x_start = x_start+1651
    data_raw = pd.read_csv(
    ...
    for i in range(x_start+1,x_start+x_split):
...
def import_real_train(): # 40 datasets
    return import_real(0, 40)

def import_real_test(): #10 datasets
    return import_real(40, 10)

def import_fake_train():   
    return import_fake(0, 40)
